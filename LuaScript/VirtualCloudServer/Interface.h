...
CCAPI CCHANDLE CreateVirtualCloudServer(int CoDataConn = 1);
CCAPI void CloseVirtualCloudServer(CCHANDLE hVirtualCloudServer);
CCAPI void SetServerAddress(CCHANDLE hVirtualCloudServer, char* ServerAddress);
CCAPI void SetCmdPort(CCHANDLE hVirtualCloudServer, int CmdPort);
CCAPI void SetDataPort(CCHANDLE hVirtualCloudServer, int DataPort);
CCAPI bool Connect(CCHANDLE hVirtualCloudServer);
CCAPI void Disconnect(CCHANDLE hVirtualCloudServer);
CCAPI bool PostIO(CCHANDLE hVirtualCloudServer, int FuncID, int Location, int Sync, char* DriverName, char* DeviceName, unsigned int Object, int MajorCmd, int MinorCmd, unsigned char *pInBuffer, unsigned int InBufferLen, PIOCOMP pNotifier, void *UserData,int TimeOut=3000 ,ZipAlgorithm CompressMode = ZIP_ALGORITHM_NONE); 
// you need free pOutBuffer and pErrorInfo after this call
CCAPI bool StartIO(CCHANDLE hVirtualCloudServer, int FuncID, int Location, int Sync, char* DriverName, char* DeviceName, unsigned int &Object, int MajorCmd, int MinorCmd, unsigned char *pInBuffer, unsigned int InBufferLen, unsigned char* &pOutBuffer,unsigned int &OutBufferLen, unsigned char* &pErrorInfo,unsigned int &ErrorInfoLen, unsigned int &ErrorCode, IOState &StateCode, int TimeOut = 30000, ZipAlgorithm CompressMode = ZIP_ALGORITHM_NONE);
CCAPI void SetUsername(CCHANDLE hVirtualCloudServer, char* Username);
CCAPI void SetPassword(CCHANDLE hVirtualCloudServer, char* Password);
CCAPI void SetVirtualDriverName(CCHANDLE hVirtualCloudServer, char* VirtualDriverName);
CCAPI void SetVirtualDeviceName(CCHANDLE hVirtualCloudServer, char* VirtualDeviceName);
CCAPI void SetVitrualMajorCmd(CCHANDLE hVirtualCloudServer, int VitrualMajorCmd);
CCAPI void SetVitrualMinorCmd(CCHANDLE hVirtualCloudServer, int VitrualMinorCmd);
CCAPI void SetPrePackageCount(CCHANDLE hVirtualCloudServer, int PrePackageCount);
CCAPI bool RegServer(CCHANDLE hVirtualCloudServer,int FuncID,int Location,int Sync,char* DriverName,char* DeviceName,PIOCOMPEX pNotifier,CCHANDLE pSender,char** ErrMsg);
CCAPI bool RegAsyncServer(CCHANDLE hVirtualCloudServer,int FuncID,int Location,int Sync,char* DriverName,char* DeviceName,PIOCOMPEX pPreProcessFunc,PIOCOMPEX pPostProcessFunc,CCHANDLE pSender,char** ErrMsg,int SubscribingMajorCmd=-1,int SubscribingMinorCmd=-1,int UnsubscribingMajorCmd=-1,int UnsubscribingMinorCmd=-1);
CCAPI bool SetTriggerEvent(CCHANDLE hVirtualCloudServer,int FuncID,int Location,int Sync,char* DriverName,char* DeviceName,DWORD nQueueHandle,DWORD nRequestID,void * pBuffer,int nBufferLen);
